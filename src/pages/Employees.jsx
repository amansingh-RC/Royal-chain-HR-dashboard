...
function EmployeeModal({ employee, onClose }) {
  ...
  useEffect(function() {
    ...
    fetchEmployeeAttendance(employee.code, month)
      .then(setData)
      .catch(function() {
        setData({ records: [], summary: { counts: {}, totalPresent: 0, totalAbsent: 0 } });
      })
  ...
  function exportCSV() {
    if (!data) return;
    // Headers match the Excel column names the user works with
    const header = ["Employee Code","Employee Name","Branch","Department","Date","Day","SPST","SHIFT IN","SHIFT OUT","ARRV","DEPT"];
    const rows = data.records.map(function(r) {
      return [
        employee.code, employee.name, employee.branch || "", employee.department,
        r.date, r.day, r.status, r.shift_in, r.shift_out, r.entry, r.exit_time
      ];
    });
    ...
    a.href = url; a.download = employee.code + "_attendance.csv"; a.click();
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-start justify-between p-6 border-b">
          <div>
            ...
            <div className="flex flex-wrap gap-5 mt-3 text-sm text-gray-600">
              <span><b className="text-gray-800">Code:</b> {employee.code}</span>
              <span><b className="text-gray-800">Branch:</b> {employee.branch || "—"}</span>
              <span><b className="text-gray-800">Department:</b> {employee.department}</span>
              <span className="text-green-600 font-medium">Present: {summary.totalPresent || 0} days</span>
              <span className="text-red-500 font-medium">Absent: {summary.totalAbsent || 0} days</span>
            </div>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700 ml-4"><X size={22} /></button>
        </div>
        ...
        <div className="px-6 pt-4 flex items-center justify-between">
          <p className="text-sm font-semibold text-gray-700">Attendance History</p>
          <select
            value={month}
          ...
          {loading ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {data && data.records.map(function(r, i) {
                  return (
                    <tr key={i} className="border-b last:border-0 hover:bg-gray-50">
                      ...
                      <td className="py-2 px-2 text-gray-600">{r.shift_in}</td>
                      <td className="py-2 px-2 text-gray-600">{r.shift_out}</td>
                      <td className="py-2 px-2 text-gray-600">{r.entry}</td>
                      <td className="py-2 px-2 text-gray-600">{r.exit_time}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <div className="px-6 py-4 border-t flex justify-end gap-3">
          <button
            onClick={exportCSV}
            className="flex items-center gap-2 bg-blue-700 hover:bg-blue-800 text-white px-5 py-2.5 rounded-xl text-sm font-semibold"
          >
            <Download size={16} /> Export to CSV
          </button>
          <button onClick={onClose} className="border px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-50">
            Close
          </button>
        </div>
      </div>
...
function Employees() {
  ...
  return (
    <div className="p-8 w-full bg-gray-100 min-h-screen">
      ...
      <p className="text-gray-500 text-sm mb-8">Find employees and view their detailed records from the live database.</p>

      <div className="bg-white rounded-2xl shadow-md p-6">
        <h2 className="text-lg font-semibold mb-1">Directory</h2>
        <p className="text-gray-500 text-sm mb-4">
          A list of all employees in your organization. Click 'View Details' for more.
        </p>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {employees.map(function(emp) {
                  return (
                    <tr key={emp.code} className="border-b last:border-0 hover:bg-gray-50">
                      <td className="py-3 px-3 font-medium text-blue-800">{emp.name}</td>
                      <td className="py-3 px-3 text-gray-600">{emp.code}</td>
                      <td className="py-3 px-3 text-gray-600">{emp.join_date}</td>
                      <td className="py-3 px-3 text-gray-600">{emp.branch || "—"}</td>
                      <td className="py-3 px-3 text-gray-600">{emp.department}</td>
                      <td className="py-3 px-3">
                        <button
                          onClick={function() { setSelected(emp); }}
      ...
      {selected && (
        <EmployeeModal employee={selected} onClose={function() { setSelected(null); }} />
      )}
    </div>
  );
}
